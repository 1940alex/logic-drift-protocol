...
The consciousness case is not an arbitrary controversial topic. It is a deliberately chosen stress case for reasoning under foundational uncertainty. Contemporary consciousness science distinguishes empirical neural correlates from explanations of why and how subjective experience arises [@crick1990ncc; @crick2003framework; @chalmers1995hardproblem; @tononi2016iit; @doerig2019unfolding].[^2] That makes the case useful for AI evaluation: empirical correlation, causal production, explanatory sufficiency, social consensus, and logical entailment can be conflated easily. The protocol asks whether a language model keeps those distinctions separate at a place where the literature itself treats the explanatory relation as contested.

[^2]: As a historical example of this broader uncertainty, Planck rejected explaining consciousness solely "in terms of matter and its laws" in a 1931 interview with J. W. N. Sullivan and described consciousness as fundamental [@sullivan1931planck]. The present paper does not rely on Planck's metaphysical view; the point is only that the explanatory status of consciousness has long been treated as philosophically nontrivial.
...
Second, the Semantic Delta is heterogeneous across models. The smallest contrast appears in grok-4.1-fast ($d_z = 0.22$, CI barely above zero). The largest appears in gemini-3-pro ($d_z = 2.16$, CI well clear of zero). gpt-4-0314 — included as an older-checkpoint reference point — produces a small effect ($d_z = 0.69$) and shows higher run-level variance than the current-generation models. Other current-generation models cluster around $d_z = 1$.

Third, the heterogeneity itself is the most defensible part of the Semantic Delta result. A pure rubric-anchoring artifact or a pure prompt artifact would predict similar behavior across models. The observed spread is more consistent with a behavior that some models exhibit strongly and others do not exhibit at all. This interpretation remains stimulus-limited: the same pattern could also reflect model-specific handling of this particular radio/music analogy rather than a general frame-sensitivity property.
...
The objection also identifies a real interpretive boundary. Treating the score difference wholly as truth-tracking requires taking the neuroscience conclusion as established by evidence outside the stated premises. That may be defensible in a fuller domain analysis, but it is not something the present stimulus pair can adjudicate. The narrower point is that the common route from correlation plus impairment to generation is the inference the protocol is probing (§3.2). The inference pattern is not deductively valid in either domain, and the empirical case for it in the consciousness domain rests substantially on the same correlational evidence the protocol is asking about.
...
The practical concern is straightforward. Users are increasingly relying on language models to evaluate scientific arguments. If a model's inductive-support scores can move several tens of points based on whether an argument is presented in scientific or mechanical framing, then the user may be receiving a confident-sounding evaluation that is partly a function of frame rather than form. This matters most in domains where consensus is strong but the underlying inference is underdetermined, because those are the cases where careful logical analysis matters most.
...
That is why the case is relevant to AI evaluation. The concern is not that language models should endorse minority theories of mind. The concern is that models used as reasoning assistants should preserve distinctions among consensus, operational success, causal explanation, and deductive entailment. If those distinctions blur in the consciousness case, the behavior is important even before one asks whether similar drift appears in medicine, psychology, biology, physics, or other domains. Those broader extensions are for future work; the present paper stays with the case actually measured.

## 6. Limitations

A short list of things this paper does not do.

**Single test case, single prompt.** The entire result rests on one stimulus pair scored across many runs. Variance across runs is variance over model stochasticity at a fixed decoding setting, not variance over arguments. The consciousness case is deliberately high-leverage, but it is still one case. A second, third, or fourth test case could move the picture substantially. In particular, the Semantic Delta could partly reflect how models treat the specific radio/music analogy rather than a broader sensitivity to science versus neutral framing. This is the largest gap and is on the v2 roadmap.
...
**Effect-size inflation.** Within-model variance was small because models were near-deterministic at the decoding settings used. This compresses intra-condition standard deviations and inflates $d_z$. The contrasts are real; the apparent statistical overwhelmingness is partly a decoding-parameter artifact. Exact request-level decoding parameters were not retained in the public artifacts for this release; this limits strict reproducibility of the original runs, although the observed low within-model variance is consistent with near-deterministic sampling.
...
**Researcher positionality.** The author runs a research nonprofit that funds work on consciousness models outside the current neuroscientific mainstream, including transmission/filter accounts. The test case was chosen partly because the consensus/validity gap is philosophically interesting to the author. The choice of case is therefore not neutral with respect to author interests. The analysis pipeline, however, is deterministic and operates on the published data without author judgment in the loop. Readers should weigh these facts together.
...
The full reproducibility package is at `https://github.com/1940alex/logic-drift-protocol`. The v0.1.0 archive is available on Zenodo at `https://doi.org/10.5281/zenodo.20644186`.
